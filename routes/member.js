...
app.post('/signin', function(req, res) {
  ...
  if(check_mail(email)){
    let query_user_data = "SELECT `password` FROM user WHERE `email`='"+email+"';"
    mysql.pool.getConnection(function(error, connection) {
      if(error){
        res.send({error:"Error in connection database."});
      }
...
app.post('/', function(req, res) {
  ...
  jwt.verify(token, secret, function (err, decoded) {
    if (err) {
      ...
      let like_image_data =[];   
      let query = 'SELECT A.`image_id` ,`image_url`,`image_source_url`,`user_photo`,D.`name`,D.`email` from `image_data` AS A RIGHT JOIN `image_like` AS B ON A.`image_id` = B.`image_id` INNER JOIN `user` AS D on D.`email`= B.`email` WHERE D.`email` = "'+tokenResult+'";'  
      mysql.pool.getConnection(function(error, connection) {
        if(error){
          console.log(error);
          res.send({error:"Error in connection database."});
        ...
        connection.query(query, function(error, result, fields){
          ...
          if(error){
            ...
            let user_query = 'SELECT `name`,`email`,`user_photo` from `user` WHERE `email`="'+tokenResult+'"'
              ...
              connection.commit(function(error){
                if(error){
                  ...
                  upload_to_S3(req.file.buffer,file_name,userId).then (function(result){
                    console.log({status:200,note:"Update photo succesd."});
                  });
